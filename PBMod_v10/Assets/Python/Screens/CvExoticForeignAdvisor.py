...
#		ExoticForPrint ("Entered drawInfo")
		...
		for iLoopPlayer in range(gc.getMAX_PLAYERS()):
			...
			if (gc.getPlayer(iLoopPlayer).isAlive() and iLoopPlayer != self.iActiveLeader
				and (gc.getTeam(gc.getPlayer(iLoopPlayer).getTeam()).isHasMet(gc.getPlayer(self.iActiveLeader).getTeam()) or gc.getGame().isDebugMode())
				and not gc.getPlayer(iLoopPlayer).isBarbarian()
				and not gc.getPlayer(iLoopPlayer).isMinorCiv()
				and not gc.getPlayer(iLoopPlayer).isWatchingCiv()):
...
#					ExoticForPrint ("Player = %d" % iLoopPlayer)
					...
					for nHost in range(gc.getMAX_PLAYERS()):
						if (gc.getPlayer(nHost).isAlive()
							...
							and (gc.getTeam(gc.getPlayer(nHost).getTeam()).isHasMet(gc.getPlayer(self.iActiveLeader).getTeam()) or gc.getGame().isDebugMode())
							and not gc.getPlayer(nHost).isBarbarian()
							and not gc.getPlayer(nHost).isMinorCiv()
							and not gc.getPlayer(nHost).isWatchingCiv()):
		...
		for iLoopBonus in range(gc.getNumBonusInfos()):
			...
			for iLoopPlayer in range(gc.getMAX_PLAYERS()):
				...
				if (currentPlayer.isAlive() and not currentPlayer.isBarbarian()
					...
					and not currentPlayer.isWatchingCiv()
