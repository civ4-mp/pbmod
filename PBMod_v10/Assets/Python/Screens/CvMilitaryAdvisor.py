...
class CvMilitaryAdvisor:
	...
	def refreshUnitSelection(self, bReload):
		...
		if bReload:
			...
			for iPlayer in range(gc.getMAX_PLAYERS()):
				...
				if (player.isAlive()
					and (not gc.getPlayer(iPlayer).isWatchingCiv() or iPlayer == self.iActivePlayer)):
	...
	def refresh(self, bReload):
		...
		for iLoopPlayer in range(gc.getMAX_PLAYERS()):
			...
			if (player.isAlive()
				and ((gc.getTeam(player.getTeam()).isHasMet(gc.getPlayer(self.iActivePlayer).getTeam())
						and (not player.isWatchingCiv() or iLoopPlayer == self.iActivePlayer))
